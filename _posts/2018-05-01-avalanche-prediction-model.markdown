...
tags: [Python]
---
This is a Python-based application that takes in historical weather data from winter 2017/2018 and elevation data from NASA’s Shuttle Radar Topgraphy Misson (SRTM). The program then creates a risk analysis of any region in the greater Salt Lake City area on any date in the winter season. It contains a command line GUI for the user to input the name of the area along with four latitude and longitude bounds. The resulting output yields an interactive 2D and 3D map for the user to see where the safest ski area. An example for Park City, Utah for April 1, 2018 in 3D is seen below
